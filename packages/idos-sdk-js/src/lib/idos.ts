...
export class idOS {
  ...
  private constructor({
    ...
    if (!enclaveOptions && !container) throw new Error("Either `container` or `enclaveOptions` must be provided");
    this.enclave = new Enclave(this.auth, new IframeEnclave(enclaveOptions ?? { container: container! }));
