...
import { CredentialValidationError } from "../schemas/utils";
...
describe("parseCredential", () => {
  it("returns a deserialized KYC v3 container", async () => {
    ...
    expect(parsed.subject.id).toBe("uuid:abc");
  ...
  it("returns a deserialized FaceId v1 container", async () => {
    ...
    expect(parsed.subject.faceSignUserId).toBe("11111111-1111-1111-1111-111111111111");
  });
  ...
  it("returns a deserialized KYC v1 container", async () => {
    ...
    expect(parsed.subject.id).toBe("uuid:v1");
    expect(parsed.subject.firstName).toBe("Ada");
    expect(parsed.subject.idDocument).toMatchObject({
      country: "US",
      number: "123456789",
      type: "PASSPORT",
  ...
  it("returns a deserialized KYC v2 container", async () => {
    ...
    expect(parsed.subject.id).toBe("uuid:v2");
    expect(parsed.subject.firstName).toBe("Grace");
    expect(parsed.subject.dateOfBirth).toEqual(new Date("1906-12-09T00:00:00.000Z"));
    expect(parsed.subject.selfieFile).toEqual(Buffer.from("Selfie"));
  });

  it("returns a deserialized EDD v1 container", async () => {
    const parsed = await parseCredential(
      credential(CREDENTIALS_V1_CONTEXT, {
        "@context": [EDD_V1_SUBJECT_CONTEXT],
        id: "uuid:edd",
        eddOccupation: "AGRICULTURE",
        eddSourceOfFundsCategory: "SALARY",
        eddSourceOfFundsProofFile: PROOF_FILE,
      }),
    );

    expect(parsed).toBeInstanceOf(VerifiableCredentialEddV1);
    if (!(parsed instanceof VerifiableCredentialEddV1)) throw new Error("expected edd v1");
    expect(parsed.subject.id).toBe("uuid:edd");
    expect(parsed.subject.edd?.occupation).toBe("AGRICULTURE");
    expect(parsed.subject.edd?.sourceOfFundsProofFile?.toString()).toBe("Proof");
  });
  ...
  it("rejects a subject field the matched context does not define", async () => {
    await expect(
      parseCredential(
        credential(CREDENTIALS_V2_CONTEXT, {
          "@context": [KYC_V3_SUBJECT_CONTEXT],
