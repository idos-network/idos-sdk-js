...
export function Onboarding() {
  const { isleController: isle } = useIsleController();
  ...
  }, [isle, address, kycDisclosure, idvStatus, signMessageAsync, queryClient]);
  ...
  const handleKYCJourneySuccess = useCallback(
    async (data: { token: string }) => {
      const [error] = await goTry(async () => {
        invariant(userData, "`userData` is not defined");
