# Dapp example

![License](https://img.shields.io/badge/license-MIT-blue?&logo=data:image/svg%2bxml;base64,)

A sample dapp demonstrating SDK usage. Uses [ethers.js](https://docs.ethers.org/) and [MetaMask](https://metamask.io/).

To run:
1. `yarn` and `yarn build`
2. serve the build through a web server (e.g. `python -m http.server` from within the `dist` folder)
3. visit `localhost:8000` (replace with your chose port)
